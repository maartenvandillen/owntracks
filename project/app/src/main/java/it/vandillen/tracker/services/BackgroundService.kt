...
import it.vandillen.tracker.App.Companion.NOTIFICATION_GROUP_EVENTS
import it.vandillen.tracker.App.Companion.NOTIFICATION_ID_EVENT_GROUP
import it.vandillen.tracker.App.Companion.NOTIFICATION_ID_ONGOING
...
import it.vandillen.tracker.di.CoroutineScopes
import it.vandillen.tracker.geocoding.GeocoderProvider
import it.vandillen.tracker.location.LatLng
import it.vandillen.tracker.location.LocationAvailability
import it.vandillen.tracker.location.LocationCallback
import it.vandillen.tracker.location.LocationProviderClient
import it.vandillen.tracker.location.LocationRequest
import it.vandillen.tracker.location.LocationResult
import it.vandillen.tracker.location.LocatorPriority
import it.vandillen.tracker.location.geofencing.Geofence
import it.vandillen.tracker.location.geofencing.GeofencingClient
import it.vandillen.tracker.location.geofencing.GeofencingEvent
import it.vandillen.tracker.location.geofencing.GeofencingEvent.Companion.fromIntent
import it.vandillen.tracker.location.geofencing.GeofencingRequest
import it.vandillen.tracker.location.toLatLng
...
import it.vandillen.tracker.preferences.Preferences.Companion.PREFERENCES_THAT_WIPE_QUEUE_AND_CONTACTS
import it.vandillen.tracker.preferences.types.ConnectionMode
import it.vandillen.tracker.preferences.types.MonitoringMode
import it.vandillen.tracker.preferences.types.MonitoringMode.Companion.getByValue
import it.vandillen.tracker.services.worker.Scheduler
import it.vandillen.tracker.support.DateFormatter.formatDate
...
class BackgroundService : LifecycleService(), Preferences.OnPreferenceChangeListener {
      ...
      when (intent.action) {
        ...
        INTENT_ACTION_CHANGE_MONITORING -> {
          if (intent.hasExtra("monitoring")) {
            ...
            val newLocationIntervalString = intent.getStringExtra("locationInterval")
            if (newLocationIntervalString != null) {
              val newLocationInterval = newLocationIntervalString.toIntOrNull()
              if (newLocationInterval != null) {
                preferences.moveModeLocatorInterval = newLocationInterval
              }
            }
          } else {
  ...
  companion object {
    ...
    const val INTENT_ACTION_SEND_LOCATION_USER = "it.vandillen.tracker.SEND_LOCATION_USER"
    const val INTENT_ACTION_SEND_EVENT_CIRCULAR = "it.vandillen.tracker.SEND_EVENT_CIRCULAR"
    private const val INTENT_ACTION_CLEAR_NOTIFICATIONS =
        "it.vandillen.tracker.CLEAR_EVENT_NOTIFICATIONS"
    private const val INTENT_ACTION_CLEAR_CONTACTS = "it.vandillen.tracker.CLEAR_CONTACTS"
    const val INTENT_ACTION_CHANGE_MONITORING = "it.vandillen.tracker.CHANGE_MONITORING"
    private const val INTENT_ACTION_EXIT = "it.vandillen.tracker.EXIT"
